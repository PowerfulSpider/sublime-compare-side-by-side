        ...
        if count['compare'] > 0 and count['notCompare'] == 0:
            ...
            def after_close():
                ...
                if count['nonCompareWin'] is None:
                    ...
                    sublime.set_timeout(
                        lambda: win.show_quick_panel(['Please close all comparison windows first'], None), 10)
...
class SbsCompareCommand(sublime_plugin.TextCommand):
    def generate_colour_scheme(self, view, generate=True):
        ...
        if int(sublime.version()) < 3000 or notHex:
            return {'removed': colour_removed, 'added': colour_added, 'modified_deletion': colour_modified_deletion,
        ...
        for col in [['removed', colour_removed], ['added', colour_added],
                    ['modified_deletion', colour_modified_deletion], ['modified_addition', colour_modified_addition]]:
    ...
    def compare_views(self, view1, view2):
        ...
        for line in diff:
            ...
            if code == '- ':
                bufferB.append('')
                if linesA[lineA].strip() != '':
                    highlightA.append(lineNum - 1)
                    bufferA.append(linesA[lineA] + '    @@ ' + str(lineA + 1) + ' @@')
                else:
                ...
                bufferA.append('')
                if linesB[lineB].strip() != '':
                    highlightB.append(lineNum - 1)
                    bufferB.append(linesB[lineB] + '    @@ ' + str(lineB + 1) + ' @@')
                else:
        ...
        if sbs_settings().get('line_count_popup', False):
            ...
            sublime.message_dialog(
                intraDiff + str(len(highlightA)) + ' lines removed, ' + str(len(highlightB)) + ' lines added\n' + str(
                    numDiffs) + ' line differences total')
